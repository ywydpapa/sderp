...
public class HomeController {
	...
	public ModelAndView modal(@RequestParam Map<String, Object> params, Model model, ModelAndView mav,
		...
		if ("cust".equals(popId)) {
			...
			String regSDate_modal = String.valueOf(params.get("regSDate_modal")).equals("")
					|| String.valueOf(params.get("regSDate_modal")).equals("null") ? now.getYear() + "-01-01"
							: String.valueOf(params.get("regSDate_modal"));
			String regEDate_modal = String.valueOf(params.get("regEDate_modal")).equals("")
					|| String.valueOf(params.get("regEDate_modal")).equals("null") ? now.getYear() + "-12-31"
							: String.valueOf(params.get("regEDate_modal"));
			...
			if (params.get("modalType").equals("cust") && !params.get("modalFlag").equals("0")) {
				String vatIssueDateFrom = String.valueOf(params.get("vatIssueDateFrom")) != ""
						? String.valueOf(params.get("vatIssueDateFrom"))
						: now.getYear() + "-01-01";
				String vatIssueDateTo = String.valueOf(params.get("vatIssueDateTo")) != ""
						? String.valueOf(params.get("vatIssueDateTo"))
						: now.getYear() + "-12-31";
	...
	public ModelAndView modal1(@RequestParam Map<String, Object> params, Model model, ModelAndView mav,
			HttpSession session, HttpServletRequest servletRequest) {
		String rtn = "";
	...
	public ModelAndView modal2(@RequestParam Map<String, Object> params, Model model, ModelAndView mav,
			HttpSession session, HttpServletRequest servletRequest) {
		String rtn = "";
	...
	public ResponseEntity<?> insertImage(HttpSession session, @ModelAttribute ImageDTO dto) {
		Map<String, Object> param = new HashMap<>();
		UUID uuid = UUID.randomUUID();
		dto.setUuid(uuid.toString());
		dto.setUserNo(session.getAttribute("userNo").toString());
		int result = utilService.insertImage(dto);
		if (result > 0) {
			param.put("code", "10001");
			param.put("url", uuid.toString()); // UUID를 문자열로 변경
		} else {
	...
	@RequestMapping("/getBase64.do")
	public ResponseEntity<?> getBase64(HttpSession session, @RequestParam String uuid) {
		Map<String, Object> param = new HashMap<>();
		...
		if (base64Data.length() > 0) {
			param.put("code", "10001");
			param.put("base64Data", base64Data); // UUID를 문자열로 변경
		} else {
			param.put("code", "20001");
		}
		return ResponseEntity.ok(param);
