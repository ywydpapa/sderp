...
public class schedController {
	...
	public ModelAndView schedrep(HttpSession session, HttpSession session2, HttpSession session3, ModelAndView mav,
			@ModelAttribute SchedDTO dto) {
	...
	public ModelAndView detailMreport(@PathVariable("sreportNo") int sreportNo, HttpSession session, ModelAndView mav,
			@ModelAttribute SchedDTO dto) {
	...
	public ModelAndView detailMreport2(@PathVariable("sreportNo") int sreportNo, HttpSession session, ModelAndView mav,
			@ModelAttribute SchedDTO dto) {
		mav.addObject("rlist", schedService.detailSreport(session, dto));
		mav.addObject("addtxt", schedService.detailAddtext(sreportNo));
	...
	public ModelAndView detail(@PathVariable("schedNo") int schedNo,
			@RequestParam(value = "simple", required = false) String simple, ModelAndView mav, HttpSession session) {
	...
	public ModelAndView detail2(@PathVariable("schedNo") int schedNo,
			@RequestParam(value = "simple", required = false) String simple, ModelAndView mav, HttpSession session) {
		mav.addObject("dto", schedService.detailSched(schedNo));
		mav.addObject("schedtype", codeService.listSchedtype(session));
		mav.addObject("acttype", codeService.listActtype(session));
	...
	public ResponseEntity<?> insSreport(@RequestBody Map<String, Object> payload, HttpSession session,
			@ModelAttribute SchedDTO dto) {
		Map<String, Object> param = new HashMap<>();
		Gson gson = new Gson();
	...
	public ResponseEntity<?> insSreport2(@RequestBody Map<String, Object> payload, HttpSession session,
			@ModelAttribute SchedDTO dto) {
		Map<String, Object> param = new HashMap<>();
		Gson gson = new Gson();
		String jsonStr = gson.toJson(payload.get("checkData"));
