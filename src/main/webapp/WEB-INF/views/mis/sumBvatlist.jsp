...
<html>
  ...
  <div id="main_content">
                  ...
                  <div class="col-sm-12 col-xl-3">
                    ...
                    <div class="input-group input-group-sm mb-0">
                      <input
                        ...
                        name="vatSellerName"
                        id="vatSellerName"
                        value="${param.vatSellerName}"
                        readonly
                      />
                      <input
                        type="hidden"
                        name="vatSellerCustNo"
                        id="vatSellerCustNo"
                        value="${param.vatSellerCustNo}"
                      />
            ...
            <div class="table-responsive">
              ...
              <table class="table table-striped table-bordered nowrap" style="margin-bottom: 1%">
                <c:forEach items="${vatList}" var="vlist">
                  ...
                  <c:set
                    var="balanceSum"
                    value="${balanceSum + ((vlist.custBalance + vlist.vatAmountB) - vlist.serialTotalB)}"
                  />
                </c:forEach>
                <thead>
                  <tr>
              ...
              <table id="vatTable" class="table table-striped table-bordered nowrap">
                ...
                <tbody>
                  <c:forEach items="${vatList}" var="vlist">
                    <c:if
                      test="${vlist.vatAmountB > 0 || vlist.serialTotalB > 0 || vlist.custBalance > 0}"
                    >
                      <tr>
                        <td class="text-center">
                          <a
                            data-remote="${path}/modal/popup.do?popId=custVatListModal&modalType=cust&custNo=${vlist.custNo}&compNo=${sessionScope.compNo}&vatIssueDateFrom=${param.vatIssueDateFrom}&vatIssueDateTo=${param.vatIssueDateTo}&modalFlag=1&vatType=B"
                            type="button"
                            data-toggle="modal"
                        ...
                        <td class="text-right">
                          <c:choose>
                            <c:when test="${vlist.vatAmountB > 0}"
                              ><fmt:formatNumber
                                type="number"
                                maxFractionDigits="3"
                                value="${vlist.vatAmountB}"
                            /></c:when>
                        ...
                        <td class="text-right">
                          <c:choose>
                            <c:when test="${vlist.serialTotalB > 0}"
                              ><fmt:formatNumber
                                type="number"
                                maxFractionDigits="3"
                                value="${vlist.serialTotalB}"
                            /></c:when>
                            <c:otherwise>0</c:otherwise>
                          </c:choose>
                        </td>
                        <td class="text-right">
                          <a
                            data-remote="${path}/modal/popup.do?popId=custVatListModal&modalType=balance&custNo=${vlist.custNo}&compNo=${sessionScope.compNo}&modalFlag=0&vatType=B"
                            type="button"
                            data-toggle="modal"
                            data-target="#custVatList"
                            style="cursor: pointer; text-decoration: underline"
                          >
                            <c:choose>
                              <c:when
                                test="${(vlist.custBalance + vlist.vatAmountB) - vlist.serialTotalB ne ''}"
                              >
                                <fmt:formatNumber
                                  type="number"
                                  maxFractionDigits="3"
                                  value="${(vlist.custBalance + vlist.vatAmountB) - vlist.serialTotalB}"
                                />
                              </c:when>
                              <c:otherwise>0</c:otherwise>
                            </c:choose>
    ...
    <script>
      ...
      function fnListcon() {
        var vatData = {};
        vatData.vatSellerCustNo = $("#vatSellerCustNo").val()
          ? Number($("#vatSellerCustNo").val())
          : 0;
