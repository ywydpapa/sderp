          ...
          <div class="col-lg-12">
            <div class="page-header-title" style="float: left">
              <div style="margin-top: 15px">
                <h6 style="font-weight: 600">미수금 현황</h6>
              </div>
            </div>
            <div class="btn_wr" style="float: right">
              ...
              <button
                ...
                onClick="javascript:location='${path}/acc/vatupload.do'"
              >
            ...
            <div class="table-responsive">
              ...
              <table class="table table-striped table-bordered nowrap" style="margin-bottom: 1%">
                <c:forEach items="${vatList}" var="vlist">
                  ...
                  <c:set var="amountSum" value="${amountSum + vlist.vatAmountS}" />
                  <c:set var="totalSum" value="${totalSum + vlist.serialTotalS}" />
                  <c:set
                    var="balanceSum"
                    value="${balanceSum + ((vlist.custBalance + vlist.vatAmountS) - vlist.serialTotalS)}"
                  />
                </c:forEach>
                <thead>
                  <tr>
                    <th class="text-center">금년도 매출합계</th>
                    <th class="text-center">금년도 수금완료합계</th>
              ...
              <table id="vatTable" class="table table-striped table-bordered nowrap">
                ...
                <c:forEach items="${vatList}" var="vlist">
                  <c:if
                    test="${vlist.vatAmountS > 0 || vlist.serialTotalS > 0 || vlist.custBalance > 0}"
                  >
                    <tr>
                      <td class="text-center">
                        <a
                          data-remote="${path}/modal/popup.do?popId=custVatListModal&modalType=cust&custNo=${vlist.custNo}&compNo=${sessionScope.compNo}&vatIssueDateFrom=${param.vatIssueDateFrom}&vatIssueDateTo=${param.vatIssueDateTo}&modalFlag=1&vatType=S"
                          type="button"
                          data-toggle="modal"
                          data-target="#custVatList"
                      ...
                      <td class="text-right">
                        <c:choose>
                          <c:when test="${vlist.vatAmountS > 0}"
                            ><fmt:formatNumber
                              type="number"
                              maxFractionDigits="3"
                              value="${vlist.vatAmountS}"
                          /></c:when>
                      ...
                      <td class="text-right">
                        <c:choose>
                          <c:when test="${vlist.serialTotalS > 0}"
                            ><fmt:formatNumber
                              type="number"
                              maxFractionDigits="3"
                              value="${vlist.serialTotalS}"
                          /></c:when>
                          <c:otherwise>0</c:otherwise>
                        </c:choose>
                      </td>
                      <td class="text-right">
                        <a
                          data-remote="${path}/modal/popup.do?popId=custVatListModal&modalType=balance&custNo=${vlist.custNo}&compNo=${sessionScope.compNo}&modalFlag=0&vatType=S"
                          type="button"
                          data-toggle="modal"
                          data-target="#custVatList"
                          style="cursor: pointer; text-decoration: underline"
                        >
                          <c:choose>
                            <c:when
                              test="${((vlist.custBalance + vlist.vatAmountS) - vlist.serialTotalS) ne ''}"
                            >
                              <fmt:formatNumber
                                type="number"
                                maxFractionDigits="3"
                                value="${(vlist.custBalance + vlist.vatAmountS) - vlist.serialTotalS}"
                              />
                            </c:when>
                            <c:otherwise>0</c:otherwise>
                          </c:choose>
    ...
    <script>
      $(document).ready(function () {
        ...
        $("#vatIssueDateFrom").change(function () {
          ...
          var dateValueCom = new Date(
            dateValueArr[0],
            parseInt(dateValueArr[1]) - 1,
            dateValueArr[2]
          );
          var EdateValue = $("#vatIssueDateTo").val();
          var EdateDateArr = EdateValue.split("-");
          var EdateDateCom = new Date(
            EdateDateArr[0],
            parseInt(EdateDateArr[1]) - 1,
            EdateDateArr[2]
          );
        ...
        $("#vatIssueDateTo").change(function () {
          ...
          var SdateValueCom = new Date(
            SdateValueArr[0],
            parseInt(SdateValueArr[1]) - 1,
            SdateValueArr[2]
          );
      ...
      function fnListcon() {
        var vatData = {};
        vatData.vatBuyerCustNo = $("#vatBuyerCustNo").val()
          ? Number($("#vatBuyerCustNo").val())
          : 0;
        vatData.vatBuyerName = $("#vatBuyerName").val();
        vatData.vatIssueDateFrom = $("#vatIssueDateFrom").val()
          ? $("#vatIssueDateFrom").val()
          : null;
