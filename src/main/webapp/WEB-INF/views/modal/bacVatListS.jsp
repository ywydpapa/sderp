<%-- Created by IntelliJ IDEA. User: djkim Date: 2022-02-04 Time: 오후 12:56 To change this template
use File | Settings | File Templates. --%> <%@ page language="java" contentType="text/html;
charset=UTF-8" pageEncoding="UTF-8"%> <%@ taglib uri="http://java.sun.com/jsp/jstl/core"
  ...
  <div class="row align-items-end">
    <div class="col-lg-12">
      ...
      <div class="btn_wr" style="float: right">
        <!-- hide and show -->
        <button class="btn btn-sm btn-success" id="fold_modal" onclick="acordian_action_modal()">
          펼치기
        </button>
        <button
          class="btn btn-sm btn-success"
          id="fold2_modal"
          onclick="acordian_action1_modal()"
          style="display: none"
        >
          접기
        </button>
        <!-- hide and show -->
        <button class="btn btn-sm btn-primary" onClick="javascript:fnListcon_modal()">
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-12 col-xl-3">
              ...
              <p class="input_inline">
                <input
                  class="form-control form-control-sm col-xl-6"
                  type="date"
                  max="9999-12-30"
                  pattern="yyyy-"
                  id="regSDate_modal"
...
<div class="dt-responsive table-responsive">
  ...
  <div>
    <button class="btn btn-sm btn-primary waves-effect" onclick="select_total_price()">
      선택계산
    </button>
  </div>
  <table id="table_test" class="table table-striped table-bordered nowrap" style="display: none">
    <thead>
      <tr>
        <th>입금금액</th>
        <th>받을금액</th>
...
<script>
  ...
  function checkConnect(){
  	select_total_price();
  	console.log($('#difference_price').val());
  ...
  function pageNationModal(page, DEFAULT_NUM, reData){
  	...
  	$.ajax({
  		...
  		success:function(countResult){
  			...
  			pageHtml += "<ul class='pagination'><li class='page-item'><a class='page-link' href='#' onClick='pagePreviousModal(this);'>이전</a></li>";
  			...
  			pageHtml += "<li class='page-item'><a class='page-link' id='pageNextBtn' href='#' onClick='pageNextModal(this);'>다음</a></li></ul>";
  				...
  				success:function(result){
  					...
  					if(result.data_secound.length > 0){
  						for(var i = 0; i < result.data_secound.length; i++){
  							table_sub_Html += "<input id='difference_price_sub' class='form-control-sm' style='border: 1px solid #ccc;' type='hidden' value='" + result.data_secound[i].difference_price.toLocaleString("en-US") + "' onkeyup='inputNumberFormat(this)'>"
  									+ "<tr><td><input id='original_price' class='form-control' style='border: 1px solid #ccc;' type='text' value='" + result.data_secound[i].inAmt.toLocaleString("en-US") + "' onkeyup='inputNumberFormat(this)' readonly></td>";
  							if(result.data_secound[i].received_price == '' || result.data_secound[i].received_price == null || result.data_secound[i].received_price == 'null'){
  								table_sub_Html += "<td id='test'><input id='received_price' class='form-control' style='border: 1px solid #ccc;' type='text' value='0' onkeyup='inputNumberFormat(this)' readonly></td>"
  							}else {
  								table_sub_Html += "<td id='test'><input id='received_price' class='form-control' style='border: 1px solid #ccc;' type='text' value='" + result.data_secound[i].received_price + "' onkeyup='inputNumberFormat(this)' readonly></td>"
  							}
  							table_sub_Html += "<td id='test2'><input id='difference_price' class='form-control' style='border: 1px solid #ccc;' type='text' value='" + result.data_secound[i].difference_price.toLocaleString("en-US") + "' onkeyup='inputNumberFormat(this)'></td>"
  							if(result.data_secound[i].difference_memo == '' || result.data_secound[i].difference_memo == null || result.data_secound[i].difference_memo == 'null'){
  								table_sub_Html += "<td><input id='received_price_detail' class='form-control' style='border: 1px solid #ccc; display: none;' type='text' value='' onkeyup='inputNumberFormat(this)'></td></tr>";
  							}else {
  								table_sub_Html += "<td><input id='received_price_detail' class='form-control' style='border: 1px solid #ccc; display: none;' type='text' value='" + result.data_secound[i].difference_memo + "' onkeyup='inputNumberFormat(this)'></td></tr>";
  							}
  						}
