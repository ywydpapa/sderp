...
<form name="form2" method="post" onsubmit="return false;">
  <div style="width: 100%">
    <div style="float: right; margin-bottom: 10px">
      ...
      <button
        ...
        data-remote="${path}/modal/popup.do?popId=contVatSetModal"
        type="button"
        data-toggle="modal"
        data-target="#contVatSetModal"
      >
        설정
      </button>
    </div>
          ...
          <tr>
            ...
            <td>
              <input
                type="text"
                class="form-control"
                id="sessionCustBossname"
                value="${sessionCust.custBossname}"
              />
            </td>
          </tr>
        </tbody>
  ...
  <div class="text-right mt-3">
    <button
      ...
      onClick="javascript:location='${path}/cont/list.do'"
    >
      계약목록
    </button>
    <button type="button" class="btn btn-md btn-primary" onclick="fn_billItemInsert();">
      발행요청하기
    </button>
  </div>
      ...
      <tbody>
        <c:forEach var="row" items="${vatData01}">
          <tr>
            ...
            <td class="text-right">
              <fmt:formatNumber value="${row.vatAmount}" pattern="#,###" />
            </td>
    ...
    $("#bodyAddBtn").click(function () {
      ...
      if (number < 1) {
        ...
        for (var i = 1; i <= number; i++) {
          bodyHtml +=
            "<tr><td><input type='date' class='form-control' max='9999-12-30' id='vatTradeDate_" +
            i +
            "' name='vatTradeDate_" +
            i +
            "' value='" +
            nowDate +
            "'></td>" +
            "<td><input type='text' class='form-control form-control-sm' id='vatProductName_" +
            i +
            "' style='min-width: 80px;' value='" +
            contVatProduct +
            "'/></td>" +
            "<td><input type='text' class='form-control form-control-sm' id='vatStandard_" +
            i +
            "' style='min-width: 80px;' value='" +
            contVatStandard +
            "'/></td>" +
            "<td><input type='text' class='form-control form-control-sm vatQuan' id='vatQuan_" +
            i +
            "' style='min-width: 80px;' value='1' /></td>" +
            "<td><input type='text' class='form-control form-control-sm vatNet' id='vatNet_" +
            i +
            "' data-index='" +
            i +
            "' onchange='calReplace(this)' style='min-width: 80px;' value='" +
            parseInt(totalNet).toLocaleString("en-US") +
            "' /></td>" +
            "<td><input type='text' class='form-control form-control-sm vatAmount' id='vatAmount_" +
            i +
            "' style='min-width: 80px;' value='" +
            parseInt(totalAmt).toLocaleString("en-US") +
            "' readonly/></td>" +
            "<td><input type='text' class='form-control form-control-sm vatTax' id='vatTax_" +
            i +
            "' style='min-width: 80px;' value='" +
            parseInt(totalTax).toLocaleString("en-US") +
            "' readonly/></td>" +
            "<td><input type='text' class='form-control form-control-sm' id='vatMemo_" +
            i +
            "' style='min-width: 80px;' /></td>" +
            "<td><button type='button' class='btn btn-sm btn-danger' data-index='" +
            i +
            "' onClick='billItemDelBtn(this);'>삭제</button></td></tr>";
        }
  ...
  function fn_billItemInsert() {
    if (confirm("발행 요청하시겠습니까??")) {
      ...
      for (var i = 1; i <= tableBody.length; i++) {
        ...
        insertData.vatNet = parseInt(
          tableBody
            .find("td #vatNet_" + i)
            .val()
            .replace(/,/g, "")
        );
        insertData.vatAmount = parseInt(
          tableBody
            .find("td #vatAmount_" + i)
            .val()
            .replace(/,/g, "")
        );
        insertData.vatTax = parseInt(
          tableBody
            .find("td #vatTax_" + i)
            .val()
            .replace(/,/g, "")
