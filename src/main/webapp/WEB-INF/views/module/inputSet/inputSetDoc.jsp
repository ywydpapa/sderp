      ...
      <tr class="item1">
        ...
        <td>
          <div class="input-group input-group-sm mb-0">
            <select
              class="form-control"
              id="productSalesEstimateCustName"
              name="productSalesEstimateCustName"
            ...
            <span class="input-group-btn">
              <button
                ...
                data-remote="${path}/modal/popup.do?popId=productdataListSalesEstimateCust"
                type="button"
                data-toggle="modal"
                data-target="#productCustModal3"
              >
              ...
              <div class="modal-dialog modal-80size" role="document">
                <div class="modal-content modal-80size">
                  ...
                  <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success waves-effect" id="custRegSimple2">
                      간편추가
                    </button>
                  </div>
                  <div
                        ...
                        <tr>
                          ...
                          <td>
                            <button
                              ...
                              id="custRegSimple_custName_check2"
                            >
                              중복확인
                            </button>
                          </td>
                        ...
                        <tr>
                          ...
                          <td>
                            <input
                              type="text"
                              value=""
                              id="custRegSimple_custMemerName2"
                              style="width: 100%"
                              placeholder="미입력시 미정으로 세팅됩니다."
                            />
                          </td>
                          <td>
                            <button
                              ...
                              id="custRegSimple_custName_register2"
                            >
                              등록
                            </button>
                          </td>
              ...
              <div class="input-group input-group-sm mb-0">
                ...
                >
                  ...
                  <c:forEach var="row" items="${listProduct}">
                    <option data-no="${row.productNo}" value="${row.productName}">
                      ${row.productName}
                    </option>
                  </c:forEach>
                </select>
        ...
        <td>
          <button id="data02Addbtn" class="btn btn-success btn-sm" onClick="dataSave();">
            추가
          </button>
          <button
            id="data02Modbtn"
            class="btn btn-instagram btn-sm"
            onClick="javascript:fn_data02Update()"
          >
            수정
          </button>
        </td>
      ...
      <tr>
        <td colspan="8">
          <details>
            ...
            <p>
              매입/매출처(거래처)는 모달창을 통해 추가하셔야 됩니다. 삭제는 직접 지우시면 됩니다.
            </p>
          </details>
        </td>
      </tr>
...
<script>
    ...
    var cal =
      parseInt(
        $("#product02InSum_table")
          .html()
          .replace(/[\D\s\._\-]+/g, "")
      ) - parseInt(dataArray[$(this).attr("data-index")].productTotal);
  ...
  $(document).on("click", "#dataUpBtn", function () {
    ...
    if ($(this).attr("data-number") > 0) {
      ...
      $("#data02Title[data-flag='true']")
        .val($(this).parents("tr").find("#dataTitle").html())
        .trigger("change");
      $("#data02Netprice").val(
        parseInt(
          $(this)
  ...
  function cardDataCheck() {
    ...
    for (var i = 0; i < checkSerialArray2.length; i++) {
      if (checkSerialArray1.indexOf(checkSerialArray2[i].cardSerial) > -1) {
        ...
        resultArray = [
          checkIndex,
          checkSerialArray2[i].cardSerial,
          checkSerialArray2[i].cardProductName,
        ];
      }
    }
    ...
    if (resultArray[0] > 0) {
      ...
      cardChecks.each(function (index, item) {
        ...
        if ($(item).is(":checked") === true) {
          var cardSerial = $(item).parent().next().next().html();
          var cardProductName = $(item).parent().next().next().next().next().html();
          var division_cardTotal = parseInt(
            $(item).parent().next().next().next().next().next().html()
          );
          if (division_cardTotal < 0) {
            var cardTotal = -parseInt(
              $(item)
                .parent()
                .next()
                .next()
          ...
          qutylist.append(
            "<tr><input type='hidden' id='docAppSerial' value='" +
              cardSerial +
              "'><td id='dataDate' style='text-align:center;'>" +
              productDate +
              "</td><td id='salesCustNoN' style='text-align:center;'>" +
              productSalesEstimateCustName +
              "</td><td id='dataTitle' style='text-align:center;'>" +
              productName +
              "</td><td id='dataNetprice' style='text-align: right'>" +
              "￦" +
              parseInt(productNetprice).toLocaleString("en-US") +
              "</td><td id='dataQuanty' style='text-align: right'>" +
              parseInt(productQty).toLocaleString("en-US") +
              "</td><td id='dataAmt' style='text-align: right'>" +
              "￦" +
              parseInt(productAmount).toLocaleString("en-US") +
              "</td><td id='dataVat' style='text-align: right'>" +
              "￦" +
              parseInt(productVat).toLocaleString("en-US") +
              "</td><td id='dataTotal' style='text-align: right'>" +
              "￦" +
              parseInt(productTotal).toLocaleString("en-US") +
              "</td><td id='dataRemark'>" +
              productRemark +
              "</td><td style='text-align:center;'><button class='btn btn-sm btn-inverse' id='dataUpBtn' data-index=" +
              dataIndex +
              " data-number='0' style='margin-right:4%;'>수정</button><button class='btn btn-sm btn-danger text-center' data-index=" +
              dataIndex +
              " id='dataDelBtn'>삭제</button></td></tr>"
          );
  ...
  function dataSave() {
    ...
    var productNetprice = $("#data02Netprice")
      .val()
      .replace(/[\D\s\._\-]+/g, "");
    var productQty = $("#data02Quanty")
      .val()
      .replace(/[\D\s\._\-]+/g, "");
    var productAmount = $("#data02Amt")
      .val()
      .replace(/[\D\s\._\-]+/g, "");
    var productVat = $("#data02Vat")
      .val()
      .replace(/[\D\s\._\-]+/g, "");
    ...
    qutylist.append(
      "<tr><input type='hidden' id='docAppSerial' value=''><td id='dataDate' style='text-align:center;'>" +
        productDate +
        "</td><td id='salesCustNoN' style='text-align:center;'>" +
        productSalesEstimateCustName +
        "</td><td id='dataTitle' style='text-align:center;'>" +
  ...
  function dataUpdate() {
    ...
    if (indexNum >= 0 && indexNum !== "") {
      ...
      qutylist
        .eq(indexNum)
        .html(
          "<input type='hidden' id='docAppSerial' value=''><td id='dataDate' style='text-align:center;'>" +
            productDate +
            "</td><td id='salesCustNoN' style='text-align:center;'>" +
            productSalesEstimateCustName +
            "</td><td id='dataTitle' style='text-align:center;'>" +
  ...
  function fn_data02Insert1() {
    ...
    if ($("#docTitle").val() === "") {
      ...
      arr.length == 0 &&
      filename1 == null &&
      $("[name='contractType']:checked").val() === "BREQ"
    ) {
      alert("영수증을 첨부해주십시오.");
      return false;
    } else {
