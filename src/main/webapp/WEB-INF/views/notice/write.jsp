...
<div id="main_content">
		...
		<div class="row">
			<div class="col-sm-12">
					...
					<div class="table-responsive">
						<table class="table table-sm bst02">
							...
							<tbody>
								...
								<tr>
									<th scope="row">제목</th>
									<td colspan="3"><input type="text" class="form-control form-control-sm" id="notiTitle" name="notiTitle" placeholder=""></td>
								</tr>
								<tr>
									<th scope="row">내용</th>
									<td colspan="3"><textarea name="notiContents" id="notiContents" rows="8" class="form-control"></textarea></td>
								</tr>
							</tbody>
						</table>
					</div>
				...
				<div class="btn_wr text-right mt-3">
					...
					<button class="btn btn-md btn-inverse modal-cancel-btn" onClick="javascript:location='${path}/notice/list.do'">취소</button>
				</div>
			</div>
		</div>
	</div>
	<script>
		...
		function fn_SaveSales() {
			...
			else if (!noticeData.notiContents){
				alert("내용을 입력해주십시오.");
				return;
			}

			$.ajax({ url: "${path}/notice/insert.do", // 클라이언트가 HTTP 요청을 보낼 서버의 URL 주소 
