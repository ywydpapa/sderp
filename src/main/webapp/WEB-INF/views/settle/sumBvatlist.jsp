                            ...
                            <div class="form-group row">
                            	<div class="col-sm-12 col-xl-3">
									...
									<div class="input-group input-group-sm mb-0">
										<select class="form-control" id="vatSellerName" name="vatSellerName" onchange="autoCompleteSelect(this);">
											<option value="">선택</option>
											<c:forEach var="row" items="${listCust}">
												<option data-no="${row.custNo}" value="${row.custName}" <c:if test="${row.custName eq param.vatSellerName}">selected</c:if>>${row.custName}</option>
											</c:forEach>
										</select>
                            ...
                            <tbody>
	                            <c:forEach items="${vatList}" var="vlist">
	                            	<c:if test="${vlist.vatAmountB > 0 || vlist.serialTotalB > 0 || vlist.custBalance > 0}">
	                            		<c:if test="${(vlist.custBalance + vlist.vatAmountB) - vlist.serialTotalB > 0 && (vlist.custBalance + vlist.vatAmountB) - vlist.serialTotalB ne ''}">
			                                <tr>
				...
				if(getCount > 0){
					$.ajax({
						...
						data: {
							...
							"settleYear": settleYear,
							"settleMon": 0,
							"settleDRbalance": parseInt(item.text().replace(/,/g, "")),
						},
					});
				}else{
					$.ajax({
						url: "${path}/acc/drCustInsert.do",
						method: "post",
						async: false,
						data: {
							"compNo": compNo,
