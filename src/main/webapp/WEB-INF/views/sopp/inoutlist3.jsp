...
<table class="table table-sm bst02" id="inoutlist">
	...
	<c:forEach var="row" items="${dtodata01}">
		...
		<input type="hidden" id="hideContTitle" value="${row.contTitle}" />
		<c:if test="${row.dataType eq '1101'}">
			...
			<tr class="item1" id="${row.soppdataNo}">
				...
				<td><button class="btn btn-sm btn-danger" onClick="javascript:fn_data01delete2(${row.soppdataNo})">삭제</button></td>
			</tr>
		</c:if>
	</c:forEach>
	<tr class="item1">
